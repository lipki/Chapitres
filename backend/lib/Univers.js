...
export class Univers {
   ...
   */
  static getUniversList() {
    return Object.keys(universData).filter(key => universData[key].themes);
  }
   ...
   */
  static getThemesListFor(universeKey) {
    const universe = universData[universeKey];
    return universe?.themes ? Object.keys(universe.themes) : [];
  }

  /**
   * Obtient la liste des fiches de perso pour un univers donné.
   * @param {string} universeKey - La clé de l'univers pour lequel obtenir les fiches.
   * @returns {string[]} Un tableau des clés des fiches de l'univers spécifié.
   */
  static getFichesListFor(universeKey) {
    const universe = universData[universeKey];
    return universe?.fiches ? universe.fiches : [];
  }

  static getWords( universeKey ) {
    return universData[universeKey || "neutre"].words;
  }
   ...
   */
  static getUnivers(univerName) {
    return universData[univerName];
  }
  ...
  static getTheme(themeName) {
    for (const universe of Object.values(universData)) {
